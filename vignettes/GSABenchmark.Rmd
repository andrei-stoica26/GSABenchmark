...
We will create gene sets for three cell types found in the data, using
...
```{r}
geneSets <- list(acinarMarkers, alphaMarkers, betaMarkers)
geneSetNames <- c('acinar', 'alpha', 'beta')
```
...
```{r}
scObj <- runGSAMethods(scObj, geneSets, geneSetNames, gsaMethods)
```
...
```{r}
smr <- runBenchmark(scObj, 'label', geneSets, geneSetNames, gsaMethods)
```
